...
int scanPorts( const int sockfd, const struct sockaddr_in& destaddr,
    const std::vector< int >& ports, std::map< int, int >& portMap,
    GuardianData& guardianData )
{
    std::set< int > respondedPorts;

    for( int attempt = 0; attempt < MAX_RETRIES && respondedPorts.size() < ports.size(); attempt++ )
    {
        for( const int port : ports )
        {
            if( respondedPorts.count( port ) != 0 )
            {
                continue;
            }

            struct sockaddr_in destination = destaddr;
            destination.sin_port = htons( port );

            const char probe[] = "Hello!";
            ssize_t sent = sendto( 
                sockfd, probe, sizeof( probe ) - 1, 0,
                reinterpret_cast< const struct sockaddr* >( &destination ),
                sizeof( destination ) );
            
            if( sent != static_cast< ssize_t >( sizeof( probe ) - 1 ) )
            {
                perror( "Error: Couldn't send scan packet" );
                return -1;
            }
        }

        while( respondedPorts.size() < ports.size() )
        {
            char buffer[2048];
            struct sockaddr_in source = {};
            socklen_t sourceLength = sizeof( source );
            ssize_t received = recvfrom( sockfd, buffer, sizeof( buffer ), 0,
                reinterpret_cast< struct sockaddr* >( &source ), &sourceLength );
            ...
            if( received < 0 )
            {
                ...
                perror( "Error: No response while scanning ports" );
                return -1;
            }

            int sourcePort = ntohs( source.sin_port );
            if( std::find( ports.begin(), ports.end(), sourcePort ) == ports.end() ||
                respondedPorts.count( sourcePort ) != 0 )
            {
                continue;
            }

            if( mapToPort( static_cast< int >( received ), sourcePort, portMap ) != 0 )
            {
                continue;
            }

            respondedPorts.insert( sourcePort );
            if( sourcePort == portMap.at( 404 ) )
            {
                memcpy( &guardianData.responseHdr, buffer, sizeof( guardianData.responseHdr ) );
                memcpy( &guardianData.responseUdpHdr,
                    buffer + sizeof( guardianData.responseHdr ),
                    sizeof( guardianData.responseUdpHdr ) );
            }
        }
    }

    return respondedPorts.size() == ports.size() ? 0 : 1;
}
...
int solveSecretPort( const int sockfd, struct sockaddr_in& destaddr, SecretData& secretData )
{
    ...
    constructMessage( secretMessage, secretNumber, userNames );
...
int solveEvilPort( const int sockfd, struct sockaddr_in& destaddr, EvilBitData& evilBitData )
{
    ...
    int rawSockfd = initializeSocket( params );
...
int main( int argc, char* argv[] )  
{
    ...
    int sockfd = initializeSocket(); // create a IPv4 UDP based socket
    ...
    // Send all discovery probes first; scanPorts receives them afterward
    // and retries only ports that have not replied yet.
    int scanResult = scanPorts( sockfd, destaddr, openPorts, portMap, guardianData );
    if( scanResult != 0 )
    {
        std::cerr << "Error: Couldn't discover all puzzle ports" << std::endl;
        close( sockfd );
        exit( 1 );
    }    
